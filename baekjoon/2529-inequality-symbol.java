...
class Main {
    ...
    static void dfs(int index, String number) {
        if(index == K + 1) {
            ans.add(number);
            return;
        }
        ...
        for(int i = 0; i < 10; i++) {
            if(!visited[i]) {
                if(index == 0 || checkCondition(index - 1, number.charAt(index - 1) - '0', i)) {
                    visited[i] = true;
    ...
    static boolean checkCondition(int symbolIndex, int prev, int curr) {
